    ...
    <>
      ...
      <div className="section-backdrop" aria-hidden="true">
        <img
          ...
          decoding="async"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/5 via-black/54 to-black/82" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-14">
          ...
          <p className="mt-4 text-white/76">Real covers, real links, sample poems from each collection.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
          {books.map((b) => (
            <article key={b.id} className={`group relative overflow-hidden rounded-2xl border border-white/10 bg-black/45 shadow-xl backdrop-blur-md ${b.featured ? "ring-2 ring-amber-200/30" : ""}`}>
              {b.featured && (
              ...
              <div className="p-6">
                <p className="text-xs tracking-wider uppercase text-amber-200/85">{b.year}{b.isbn ? ` • ISBN ${b.isbn}` : ""}</p>
                <h3 className="mt-1 font-serif text-2xl font-semibold leading-snug">{b.title}</h3>
                <p className="text-sm text-white/72">{b.subtitle}</p>
                <p className="mt-3 line-clamp-4 text-sm text-white/68">{b.description}</p>
                <div className="mt-4 flex flex-wrap gap-2">
                  {b.themes.map((t) => <span key={t} className="rounded-full bg-white/8 px-2 py-1 text-xs text-white/68">{t}</span>)}
                </div>
                ...
                <div className="mt-6 space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="group inline-flex items-center justify-center gap-2 rounded-lg border border-white/15 px-4 py-2 text-sm text-white transition-colors hover:bg-white/10"
                    ...
                    <Link 
                      href={`/books/${b.id}`}
                      className="group inline-flex items-center justify-center gap-2 rounded-lg bg-amber-200 px-4 py-2 text-sm text-black transition-colors hover:bg-amber-100"
                  ...
                  {(b.buyLinks.amazon || b.buyLinks.googleBooks) && (
                    <a 
                      ...
                      className="block rounded-lg border border-amber-200 px-4 py-2 text-center text-sm text-amber-200 transition-colors hover:bg-amber-200 hover:text-black"
