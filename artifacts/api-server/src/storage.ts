...
export class MemStorage implements IStorage {
  ...
  async updateCategory(id: string, category: Partial<InsertBlogCategory>): Promise<BlogCategory | undefined> {
    ...
    const sanitized = this.removeUndefinedFields(category);
    const updated: BlogCategory = { ...existing, ...sanitized, updatedAt: new Date() };
  ...
  async updatePost(id: string, post: Partial<InsertBlogPost>): Promise<BlogPost | undefined> {
    ...
    if (!existing) return undefined;
    const sanitized = this.removeUndefinedFields(post);
    const updated: BlogPost = { ...existing, ...sanitized, updatedAt: new Date() };
  ...
  async publishPost(id: string): Promise<BlogPost | undefined> {
    ...
    const updated: BlogPost = { ...post, isPublished: true, publishedAt: new Date(), updatedAt: new Date() };
    this.posts.set(id, updated);
    return updated;
  }
